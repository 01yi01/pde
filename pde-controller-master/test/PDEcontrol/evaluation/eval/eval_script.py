...
def eval_robustness_helper(robustness, natural_language, verbose = False, reasoning = False):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    wrapper_script_path = os.path.join(current_dir, 'eval_robustness_wrapper.py')
    if reasoning:
        wrapper_script_path = os.path.join(current_dir, 'eval_robustness_reasoning_wrapper.py')
...
def eval_robustness_DPO(item, anchor_pred_key='predicted_python', interm_pred_key='predicted_intermediate_python'):
    anchor = item[anchor_pred_key]
    interm = item[interm_pred_key]
    anchor = anchor.replace('\x00', '')
    interm = interm.replace('\x00', '')
    pred_robustness, pred_runtime = eval_robustness_helper(anchor, interm, reasoning=True)
    return pred_robustness, pred_runtime
